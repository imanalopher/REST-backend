...
class AuthenticationTest extends LocalWebTestCase {
	...
	public function testLogin() {
		\User::create(['username' => "nicholasjstock", "email" => "stockn@gmail.com", "first_name" => "nicholas", "last_name" => "stock", "password" => "surfer"]);
		$parameters = array('user' => ['username' => 'nicholasjstock', "password" => "surfer"]);
		$this->client->post('/users/sign_in', $parameters);
		$this->assertEquals(200, $this->client->response->status());
	}
	public function testProtectedRoute() {
		\User::create(['auth_token' => "thoken", 'username' => "nicholasjstock", "email" => "stockn@gmail.com", "first_name" => "nicholas", "last_name" => "stock", "password" => "surfer"]);
		$header = 'Token token="thoken", user_email="stockn@gmail.com"';
		$this->client->get("/api/speakers", [], ["HTTP_Authorization" => $header]);
		$this->assertEquals(200, $this->client->response->status());
	}
